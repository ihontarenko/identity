...
/**
 * Checks {@code policy/identity.jmp} against {@link Permissions} in <strong>both</strong> directions,
 * and stops the application where they disagree.
 *
 * <p>This is the whole reason writing authorization down is safe rather than merely tidy. A permission
 * is a bare string wherever it is asked about, so a document writing {@code user:delete} where the
 * constant says {@code user:remove} would load cleanly, bind cleanly, seed a role that carries it, and
 * grant absolutely nothing — with no log line anywhere saying so. Nobody finds that until an
 * administrator cannot do something the screen offers them.
 *
 ...
 *       engine never looks a permission up — but it means the document has stopped being the
 *       vocabulary, and a document that is only <em>most</em> of the truth is one nobody trusts enough
 *       to read.
 * </ul>
 *
