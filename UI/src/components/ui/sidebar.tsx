...
function SidebarHeader({ className, ...properties }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      // inset and the brand-to-nav rhythm.
  ...
  return (
    <div
      ...
      className={cn(
        ...
        // region wheel/touch/keyboard-scrollable without a visible track+thumb.
...
function SidebarGroupLabel({
  ...
  return (
    <Component
      ...
      className={cn(
        // Uppercase + letter-spaced + bold rather than shadcn's plain small-medium default. px-2.5
        // (10px) matches menu items' own horizontal inset so labels and items align to the same left
        // edge.
...
// sidebar-accent/sidebar-accent-foreground are mapped (see index.css) to an inverted solid ink/paper
// pair for an active nav-item look, so hover needs its own, gentler treatment or every item would
// look selected on mouseover.
// font-medium is base-level (not just data-active) — shadcn's own default only bumps weight on the
// active state.
// gap-[9px]/rounded-[10px]/[&>svg]:size-[15px] and the default size's py-[7px] px-2.5 text-[13px]
// port the ported navigation item's exact values instead of shadcn's gap-2/rounded-md/size-4/h-8
// text-sm defaults.
